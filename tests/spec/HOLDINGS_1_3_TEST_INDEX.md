...
| H04 | Object-body `I HAS` declarations are object-local and retain declaration-family relaxations | `top-level-object-scope-does-not-leak`, `object-body-redeclare-overwrite-does-not-mutate-outer`, `object-body-declare-without-article-src`, `object-body-declare-no-init-default-noob-src` |
...
| H21 | `<object> HAS A <slot>` no-`ITZ` shorthand defaults to `NOOB` with full consequence set (dynamic slot names, redeclare reset, inherited/mixin local shadow) | `slot-set-no-itz-shorthand`, `slot-set-no-itz-shorthand-src`, `me-slot-no-itz-shorthand-src`, `slot-set-no-itz-shorthand-srs-slot-src`, `slot-set-no-itz-redeclare-resets-noob-src`, `slot-set-no-itz-then-explicit-init-src`, `slot-set-no-itz-inherited-shadow-src`, `slot-set-no-itz-mixin-shadow-src` |
| H22 | `OBTW` block comments are boundary-sensitive (including object-body boundaries) | `obtw-mid-command-negative`, `object-body-obtw-mid-command-negative`, `block-comment-comma-boundary-src`, `object-body-block-comment-comma-boundary-src` |
...
| H29 | Direct keyword identifiers are rejected; `SRS` can synthesize keyword-shaped names across admitted dynamic-name sites; parameter binders remain direct-only | `reserved-keyword-mkay-name`, `reserved-keyword-mkay-arg`, `srs-keyword-name-in-function-arg-src`, `srs-keyword-slot-name-src`, `srs-keyword-function-name-src`, `srs-keyword-method-name-src`, `srs-keyword-object-name-src`, `srs-keyword-receiver-name-src`, `srs-keyword-assignment-target-src`, `srs-keyword-input-target-src`, `srs-keyword-cast-target-src`, `prototype-srs-keyword-parent-name-src`, `prototype-srs-keyword-mixin-name-src`, `method-def-srs-keyword-receiver-slot-tail-src`, `srs-parameter-binder-negative`, `function-dynamic-arg-name`, `method-dynamic-arg-name`, `srs-nested-indirection-src`, `srs-generated-source-not-reparsed-src` |
| H30 | `HAI` header version token is numeric syntax; strict acceptance remains exactly `1.3` | `missing-version`, `version-token-must-be-number`, `version-string-literal-negative`, `unsupported-v12`, `unsupported-v14` |
| H31 | `KTHXBYE` hard-closes the program; no inline `BTW` tail or trailing non-whitespace material allowed | `kthxbye-inline-btw`, `one-line-minimal-inline-btw`, `trailing-btw-after-close`, `trailing-obtw-after-close`, `one-line-extra-after-close` |
| H32 | Identifier character domain is ASCII-leading-letter + ASCII letter/digit/underscore tail; mixed-case/underscore allowed, non-ASCII direct identifiers rejected | `valid-ident-mixed-case-underscore`, `invalid-ident-unicode-letter`, `invalid-ident-leading-underscore`, `invalid-ident-with-dash`, `invalid-ident-symbol-only` |
| H33 | Method-definition placement split: nested `HOW ... I ...` stays restricted; executable receiver-targeted `HOW ... <receiver> ...` is valid in statement positions | `method-def-inside-function-src`, `method-def-inside-method-src`, `how-duz-receiver-runtime-src`, `nested-function-def`, `nested-function-def-in-method` |
