...
| `N88` | adjudicated               | `SRS` at identifier-binding and lookup sites must produce identifier-shaped names at runtime; non-identifier results are runtime errors. Nested `SRS` is repeated identifier indirection (not source reparsing). Assignment/input/cast-assignment targets follow the same dynamic-identifier policy (`srs-numeric-target-src`, `srs-keyword-assignment-target-src`, `srs-keyword-input-target-src`, `srs-keyword-cast-target-src`, `srs-nested-indirection-src`, `srs-generated-source-not-reparsed-src`).                                                                                                                                                   |
| `N90` | adjudicated               | `<object> HAS A <slot>` without `ITZ` is accepted as declaration shorthand and initializes the slot to `NOOB` (canonical explicit form remains `... ITZ <expr>`). This applies generally (including `ME`) rather than as a receiver-only exception, with full consequence coverage across dynamic slot names, redeclaration reset, and inherited/mixin local shadow behavior (`slot-set-no-itz-shorthand-src`, `me-slot-no-itz-shorthand-src`, `slot-set-no-itz-shorthand-srs-slot-src`, `slot-set-no-itz-redeclare-resets-noob-src`, `slot-set-no-itz-then-explicit-init-src`, `slot-set-no-itz-inherited-shadow-src`, `slot-set-no-itz-mixin-shadow-src`, parse acceptance `slot-set-no-itz-shorthand`). |
| `N91` | adjudicated               | `OBTW` block comments are statement-boundary sensitive: recognized only at logical command start (line start or comma boundary), rejected mid-command, including object-body contexts (`obtw-mid-command-negative`, `object-body-obtw-mid-command-negative`, `block-comment-comma-boundary-src`, `object-body-block-comment-comma-boundary-src`).                                                                                                              |
...
| `N99` | adjudicated               | Method-definition placement split is explicit: nested `HOW ... I ...` remains declaration-like and restricted, while receiver-targeted `HOW ... <receiver> ...` remains executable in statement positions (`method-def-inside-function-src`, `method-def-inside-method-src`, `nested-function-def`, `nested-function-def-in-method`).                                                                                                                                                                                           |
...
## N99 Rationale (Method-Definition Placement Split)

Strict holding:

1. `HOW ... I ... IF U SAY SO` remains declaration-like and placement-restricted.
2. `HOW ... <receiver> <slot> ... IF U SAY SO` remains an executable receiver mutation statement and is valid wherever statements are valid.

Why this is coherent:

- Parser and runtime already model distinct node families (`stmt-function-def` vs `stmt-method-def`) with different execution behavior.
- Receiver-targeted definitions evaluate a receiver and mutate that object’s slot table immediately at statement execution time.
- Collapsing both forms into one placement rule would either over-restrict executable receiver mutation or erase the declaration-vs-mutation distinction that current semantics already enforce.

Pinned regressions:

- `method-def-inside-function-src`
- `method-def-inside-method-src`
- `nested-function-def`
- `nested-function-def-in-method`
